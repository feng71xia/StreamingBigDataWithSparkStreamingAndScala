...
object StructuredStreaming {
   ...
   def main(args: Array[String]) {
      ...
      val spark = SparkSession
        ...
        .master("local[*]")
        .config("spark.sql.warehouse.dir", "/tmp/spark_warehouse") // Necessary to work around a Windows bug in Spark 2.0.0; omit if you're not on Windows.
        .config("spark.sql.streaming.checkpointLocation", "/Users/fengxia/tutorial/SparkStreamingIDEA/SparkStreaming/target/SSCheckpoint")
        .getOrCreate()
        
      // setupLogging()
        
      // Create a stream of text files dumped into the logs directory
      val rawData = spark.readStream.text("/Users/fengxia/tutorial/SparkStreamingIDEA/SparkStreaming/data/logs")
